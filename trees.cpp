...
void bfs1(node*root)//direct print
{
    queue<node*> q;
    q.push(root);
    while(!q.empty())
...
void bfs2(node*root)// print in new line after each level
{
    queue<node*> q;
    q.push(root);
    q.push(NULL);
...
int count(node*root)//count number of nodes
{
    if(root==NULL)
    return 0;
     return 1+count(root->left)+count(root->right);
}
int sum(node*root)//count sum of all the nodes
